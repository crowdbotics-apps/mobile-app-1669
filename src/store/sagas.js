...
function* newplugin221_get__readWorker(action) {
  try {
    const result = yield call(apiService.newplugin221_get__read, action)
    yield put(actions.newplugin221_get__readSucceeded(result))
  } catch (err) {
    yield put(actions.newplugin221_get__readFailed(err))
  }
}
function* newplugin221_get__readWatcher() {
  yield takeEvery(types.NEWPLUGIN221_GET__READ, newplugin221_get__readWorker)
}
